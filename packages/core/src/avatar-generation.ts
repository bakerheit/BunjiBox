...
import { BUNJI_CODEX_DISABLED_FEATURES } from './codex-profile.mjs'
...
export function avatarImageData(result) {
...
export async function generateAvatar(prompt, { signal, spawnProcess = spawn, startupTimeoutMs = 30000 } = {}) {
  const cwd = await mkdtemp(join(tmpdir(), 'bunji-avatar-'))
  let child, childStopped, startupTimer, killTimer, settled = false
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      child = spawnProcess('codex', args, { cwd, env: process.env, stdio: ['pipe', 'pipe', 'pipe'] })
      childStopped = new Promise(resolve => { child.once('exit', resolve); child.once('error', resolve) })
      child.on('error', () => finish(fail('Could not start Codex. Install it and sign in with ChatGPT.')))
      child.on('exit', () => { clearTimeout(killTimer); finish(fail('Codex closed before returning an image. Please try again.')) })
      child.stderr.on('data', () => {})
      child.stdin.on('error', () => finish(fail('The Codex image connection closed.')))
...
export function createAvatarGenerations({ generate = generateAvatar, now = Date.now, retentionMs = 20 * 60 * 1000 } = {}) {
  const jobs = new Map()
  let closed = false
  const snapshot = job => ({ id: job.id, status: job.status, image: job.image, error: job.error })
  const prune = () => { for (const [id, job] of jobs) if (job.status !== 'running' && now() - job.finishedAt > retentionMs) jobs.delete(id) }
  return {
    start(body) {
      prune()
      if (closed) throw fail('Image generation is shutting down.', 503)
      if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(key => !['id', 'prompt'].includes(key)) || typeof body.id !== 'string' || !/^[a-zA-Z0-9_-]{16,80}$/.test(body.id) || typeof body.prompt !== 'string' || !body.prompt.trim() || body.prompt.length > 2000) throw fail('Enter a picture description of 1–2,000 characters and a valid request ID.')
      const prompt = body.prompt.trim(), previous = jobs.get(body.id)
      if (previous) {
      ...
      const job = { id: body.id, prompt, status: 'running', image: null, error: null, controller }
      jobs.set(job.id, job)
      job.promise = Promise.resolve().then(() => generate(prompt, { signal: controller.signal })).then(image => {
        ...
        if (job.status === 'running') { job.status = 'failed'; job.error = String(error.message || 'Image generation failed.').slice(0, 500) }
