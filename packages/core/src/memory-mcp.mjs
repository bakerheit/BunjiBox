...
export function createMemoryServer({ store, botId, sourceId, allowWrites = false, files, computer, cwd }) {
  if (!validId(botId) || !validId(sourceId)) throw new Error('Invalid memory scope.')
  ...
  register('memory_write', 'Create or update a durable, useful note when relevant to future chats. Do not save every message. Updates require the revision returned by memory_read. Never store credentials.', {
    ...
    return store.write(botId, { ...args, sourceMessageIds: [...new Set([...(previous?.sourceMessageIds || []), sourceId])].slice(-100) })
  })
  ...
  if (files) register('files_publish', 'Show a file you created or edited in this agent’s Files panel. Call after generating files with shell commands or other tools, including files saved outside the suggested output folder. Use the real file path. This registers the file without copying or changing it.', { path: z.string().min(1).max(4096) }, true,
    args => files.register(botId, sourceId, args.path, { computer, cwd }))
  return server
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const { values } = parseArgs({ options: { directory: { type: 'string' }, bot: { type: 'string' }, source: { type: 'string' }, write: { type: 'boolean', default: false }, 'files-db': { type: 'string' }, 'files-computer': { type: 'string' }, 'files-cwd': { type: 'string' } } })
