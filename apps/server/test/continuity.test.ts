...
import type { BotsSnapshot, ChatRequest, HistoryPage, MemoryNote, RewindResult, StartResult } from '@bunji/shared/types'

const settings = { provider: 'codex', model: 'gpt-5.6-luna', effort: 'low', mode: 'agent' } as const
...
test('full-Mac requests and memory writes work from any same-origin device', async t => {
  ...
  const endpoint = `http://127.0.0.1:${(server.address() as AddressInfo).port}/api/bots/bunjibox/messages`
  ...
  const memoryEndpoint = `http://127.0.0.1:${(server.address() as AddressInfo).port}/api/bots/bunjibox/memory`
...
test('service persists before acknowledgement, owns disconnected runs, and retries never rerun', async t => {
  ...
  const server = http.createServer((req, res) => void handler(req, res)); await new Promise<void>(resolve => server.listen(0, '127.0.0.1', resolve))
  ...
  assert.equal((await response.json() as StartResult).request.prompt, body.prompt)
  assert.ok((await (await fetch(base + '/api/bots/bunjibox/history')).json() as HistoryPage).requests[0].activities.some(activity => activity.title === 'memory_search'))
  ...
  assert.throws(() => f.service.start('bunjibox', { ...body, id: 'two' }), (error: any) => error.status === 409)
...
test('memory API rejects foreign writes and stale edits, preserves provenance on manual edits', async t => {
  ...
  t.after(() => new Promise(resolve => { server.close(resolve); server.closeAllConnections() }))
  const base = `http://127.0.0.1:${(server.address() as AddressInfo).port}`
  const post = (path, body, method = 'POST', origin?: string) => fetch(base + path, { method, headers: { 'content-type': 'application/json', ...(origin ? { origin } : {}) }, body: JSON.stringify(body) })
  ...
  assert.equal(edit.status, 200); assert.deepEqual((await edit.json() as { note: MemoryNote }).note.sourceMessageIds, ['source-one'])
...
test('editing saved messages and deleting an agent transfers linked memories', async t => {
  ...
  assert.equal((await edited.json() as { request: ChatRequest }).request.prompt, 'New question')
  ...
  assert.ok(!(await removed.json() as BotsSnapshot).bots.some(bot => bot.id === 'scout'))
...
test('rewind returns the target prompt and removes that turn plus later turns', async t => {
  ...
  assert.deepEqual((await rewound.json() as { rewind: RewindResult }).rewind, { id: 'rewind', botId: 'bunjibox', prompt: 'Try this again', removed: 2 })
...
test('context is bounded and explicitly reports omitted full exchanges', () => {
  const turns = Array.from({ length: 20 }, (_, index) => ({ prompt: `Question ${index}`, text: 'answer '.repeat(500), status: 'complete' as const }))
  const result = buildContext({ name: 'Agent', description: '' }, turns, 'Continue', { completedCount: 45 })
  ...
  assert.throws(() => buildContext({ name: 'Agent', description: '' }, [], 'x'.repeat(12000)), /too long/)
})
