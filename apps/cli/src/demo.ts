...
import { BunjiSession, defaultBots } from './session.ts'

// Explicit offline UI preview, never mixed with real subscription usage.
export function createDemoSession(): BunjiSession {
  return new BunjiSession({
    ...
    run: async (_options, { signal, onActivity }) => {
      ...
      const publish = (item: Activity) => { const i = activities.findIndex(value => value.id === item.id); if (i < 0) activities.push(item); else activities[i] = item; onActivity(item) }
      const wait = () => new Promise<void>(resolve => { if (signal.aborted) { resolve(); return }; const done = () => { clearTimeout(timer); signal.removeEventListener('abort', done); resolve() }; const timer = setTimeout(done, 650); signal.addEventListener('abort', done, { once: true }) })
