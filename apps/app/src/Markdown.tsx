...
function CodeBlock({ children }: { children?: ReactNode }) {
  ...
  // The fenced code element react-markdown renders inside <pre>.
  const language = (children as { props?: { className?: string } } | null | undefined)?.props?.className?.replace('language-', '') || 'code'
  const copy = async () => {
    try {
