...
export default function FilesPanel({ botId, botName, onClose, onSettings }) {
  ...
  useEffect(() => {
    if (!selected) return
    const controller = new AbortController()
    setDetail(null); setPreviewError('')
  ...
  return <section className="files-panel" aria-label={`${botName} files`}>
    ...
    {selected ? <div className="files-preview panel-scroll">
      ...
      <div className="files-scroll">
        ...
        <div className={`agent-files ${layout}`} aria-label="Agent files">{shown.map(item => <button key={item.id} className={`file-card ${category(item)}${item.available ? '' : ' unavailable'}`} title={item.path} onClick={() => setSelected(item.id)} aria-label={`Preview ${item.name}`}>
