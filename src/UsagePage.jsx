...
import { ArrowLeft, ArrowUpRight, Check, Clock3, Gauge, Info, Menu, RefreshCw, ShieldCheck, Terminal } from 'lucide-react'
...
function ProviderCard({ id, data, loading, now }) {
...
export default function UsagePage({ onMenu, onBack, setConnectionStatus }) {
  ...
  return <>
    ...
    <div className="usage-scroll"><div className="usage-content">
      <div className="usage-title"><div><span className="usage-eyebrow"><Gauge size={14} />ACCOUNT USAGE</span><h1>A little headroom.</h1><p>Your Codex and Claude limits, in one place.</p></div><Button variant="outline" className="usage-refresh" disabled={loading} onClick={() => { setLoading(true); refresh(true) }}><RefreshCw size={15} className={loading ? 'usage-spinning' : ''} />{loading ? 'Checking…' : 'Refresh'}</Button></div>
      <div className="usage-scope"><Info size={15} /><p>Account-wide usage, including activity outside BunjiBox. These are the accounts signed in on your Mac.</p></div>
      {error && <p role="alert" className="usage-error">{error}</p>}
      <div className={`usage-provider-grid${error ? ' stale' : ''}`} aria-busy={loading}>{Object.keys(providerInfo).map(id => <ProviderCard key={id} id={id} data={data?.providers?.[id]} loading={loading} now={now} />)}</div>
