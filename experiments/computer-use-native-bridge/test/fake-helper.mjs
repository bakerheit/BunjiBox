...
if (args[0] !== '--stdio' || args[1] !== '--target' || args.length !== 3 || process.env.BUNJI_NATIVE_EXPERIMENT !== '1') process.exit(2)
const target = args[2]
...
createInterface({ input: process.stdin }).on('line', line => {
  ...
  if (method === 'status') return send({ target, pid: process.pid, enabled: process.env.BUNJI_NATIVE_EXPERIMENT, stopped })
  if (method === 'stop') { stopped = true; return send({ stopped: true }) }
  if (stopped && (method === 'focus' || method === 'act')) return error('User takeover: host UI must resume')
  if (method === 'focus') return send({ target, focused: true })
  if (method === 'act') {
    if (params.frameId !== 'frame-1') return error('Stale frame')
