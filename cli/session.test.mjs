...
import { providerCommand, MAX_PROMPT_LENGTH } from '../core/runtime.mjs'
...
test('shared runner validates settings and passes prompts as single arguments without a shell', () => {
  ...
  const [ollama, ollamaArgs] = providerCommand({ provider: 'ollama', model: 'qwen3:1.7b', effort: 'low', prompt })
  assert.equal(ollama, 'ollama-http')
  assert.deepEqual(ollamaArgs, ['qwen3:1.7b'])
