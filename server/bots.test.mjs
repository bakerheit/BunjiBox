...
import { openBotStore } from '../core/bot-store.mjs'
import { defaultBots, cliBot } from '../shared/bots.js'
import { BotClient } from '../shared/bot-client.js'
import { BunjiSession } from '../cli/session.mjs'
import { createBotRoutes } from './bots.mjs'
import { botTransport, readLegacyBots } from '../src/lib/bot-api.js'
...
test('simultaneous CLI processes keep every created bot', async t => {
  const { store, path } = await fixture(t)
  const module = new URL('../core/bot-store.mjs', import.meta.url).href
